  ...
  >
    <div class="flex flex-col gap-2 justify-between">
      ...
      <strong class="text-xl">{{
        $n(data?.current_period.data[0].frequency || 0, 'decimal')
      }}</strong>
      <PercentageDisplay
    ...
    <Icon
      ...
      class="absolute text-primary -right-4 -bottom-4 text-4xl"
...
import type { PurchaseFrequencyCardData } from '@/types'
...
const data = ref<PurchaseFrequencyCardData | null>(null)

const diferenciaPorcentaje = computed(() =>
  getPercentageDifference(
    data.value?.current_period.data[0].frequency,
    data.value?.previous_period.data[0].frequency,
  ),
)
...
watch(dashboardStore, () => {
  if (!dashboardStore.cardLoading[props.dataKey]) {
    data.value = dashboardStore.cardData[props.dataKey] as PurchaseFrequencyCardData
  }
})
</script>
